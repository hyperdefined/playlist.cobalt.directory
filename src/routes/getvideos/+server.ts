...
const errorMessages = {
  ...
  catchedError:
    "something went wrong, please try again later and if this " +
...
export async function GET({ url }) {
  try {
    ...
    const soundcloudMatch = playlistUrl?.match(
      /soundcloud\.com\/[a-z0-9-_]+\/sets\/[a-z0-9-_]+/,
    );
    let playlistVideos;
    ...
    return new Response(errorMessages.catchedError + error.message, {
      status: 500,
    });
  }
}
